...
def manage():
    try:
        ...
        # Fetch username and role from the login table
        cursor.execute('SELECT username, role FROM login')
        users = cursor.fetchall()  # List of tuples with (username, role)
        
        # Render the manage.html template and pass the users list
